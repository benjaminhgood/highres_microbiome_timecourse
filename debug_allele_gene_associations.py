...
home_longgene = ('Alistipes_onderdonkii_55464', '1120974.3.peg.19')
away_longgene = ('Bacteroides_coprocola_61586', '470145.6.peg.676')
...
output_filename = 'debug'+allele_filename_items[0]
if bootstrapped:
...
desired_samples = [parse_timecourse_data.highcoverage_start_1, parse_timecourse_data.highcoverage_antibiotic, parse_timecourse_data.highcoverage_end]
...
for batch in xrange(0,num_batches):
    ...
    for sample_idx in xrange(0,len(desired_samples)):
        ...
        if home_longgene in longgene_id_map:
            home_longgene_id = longgene_id_map[home_longgene]
        else:
            sys.stderr.write("Home longgene not in map...\n")
            home_longgene_id = -1
            
        if away_longgene in longgene_id_map:
            away_longgene_id = longgene_id_map[away_longgene]
        else:
            sys.stderr.write("Away longgene not in map...\n")
            away_longgene_id = -1
        ...
        for barcode_id in barcode_longgene_ids_map:
            ...
            if barcode_depth_map[barcode_id] > 2.5: # Poor man's error correction
                ...
                if (home_longgene_id in barcode_longgene_ids_map[barcode_id]) and (away_longgene_id in barcode_longgene_ids_map[barcode_id]):
                    print barcode_id
