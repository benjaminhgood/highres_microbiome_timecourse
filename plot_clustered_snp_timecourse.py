...
for species_idx in xrange(0,len(species_names)):        
    ...
    cluster_map = cluster_utils.cluster_snps(cluster_As, cluster_Ds, max_num_snps_to_cluster=0)
        
    for cluster_id in cluster_map:
        color = None
        avg_fs = cluster_map[cluster_id]['centroid']
        line, = freq_axis.plot(times, avg_fs, '-o', alpha=1, markersize=3, markeredgecolor='none', zorder=5, linewidth=1)
        color = pylab.getp(line,'color')
