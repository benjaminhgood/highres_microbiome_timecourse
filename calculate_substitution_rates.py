...
if __name__=='__main__':
    ...
    good_species_list = parse_midas_data.parse_good_species_list(min_prevalence=2)
    ...
    file.write(", ".join(['Species', 'Sample1', 'Sample2', 'Type', 'Num_muts', 'Num_revs', 'Num_mut_opportunities', 'Num_rev_opportunities']))
    
    for species_name in good_species_list:
        record_strs = []
        sys.stderr.write("Loading highcoverage samples...\n")
        ...
        if len(snp_samples) < min_sample_size:
            sys.stderr.write("Not enough highcoverage samples!\n")
            continue
        
        sys.stderr.write("Proceeding with %d highcoverage samples!\n" % len(snp_samples))
        ...
        snp_pi_difference_matrix = numpy.array([])
        snp_pi_opportunity_matrix = numpy.array([])
    
        snp_f2_difference_matrix = numpy.array([])
        snp_f2_variance_matrix = numpy.array([])
        snp_f2_opportunity_matrix = numpy.array([])
        ...
        while final_line_number >= 0:
            ...
            # All pi
            chunk_snp_pi_difference_matrix, chunk_snp_pi_opportunity_matrix = diversity_utils.calculate_pi_matrix(allele_counts_map, passed_sites_map, allowed_genes=non_shared_genes)
            
            # All f2
            chunk_snp_f2_difference_matrix, chunk_snp_f2_variance_matrix, chunk_snp_f2_opportunity_matrix = diversity_utils.calculate_f2_matrix(allele_counts_map, passed_sites_map, allowed_genes=non_shared_genes)
            ...
            if snp_mut_difference_matrix.shape[0]==0:
                ...
                snp_pi_difference_matrix = numpy.zeros_like(snp_mut_difference_matrix)*1.0
                snp_pi_opportunity_matrix = numpy.zeros_like(snp_mut_difference_matrix)*1.0
                
                
                snp_f2_difference_matrix = numpy.zeros_like(snp_mut_difference_matrix)*1.0
                snp_f2_variance_matrix = numpy.zeros_like(snp_mut_difference_matrix)*1.0
                snp_f2_opportunity_matrix = numpy.zeros_like(snp_mut_difference_matrix)*1.0
            ...
            snp_pi_difference_matrix += chunk_snp_pi_difference_matrix
            snp_pi_opportunity_matrix += chunk_snp_pi_opportunity_matrix
            
            snp_f2_difference_matrix += chunk_snp_f2_difference_matrix
            snp_f2_variance_matrix += chunk_snp_f2_variance_matrix
            snp_f2_opportunity_matrix += chunk_snp_f2_opportunity_matrix
        ...
        for idxs in [same_sample_idxs, same_subject_idxs, diff_subject_idxs]:
            for sample_pair_idx in xrange(0,len(idxs[0])):
                
                # do both order
                for i,j in list(set([(idxs[0][sample_pair_idx], idxs[1][sample_pair_idx]), (idxs[1][sample_pair_idx], idxs[0][sample_pair_idx])])):
                    ...
                    record_str_items = [species_name, sample_i, sample_j, 'all_pi', str(snp_pi_difference_matrix[i,j]/2.0), str(snp_pi_difference_matrix[i,j]/2.0),  str(snp_pi_opportunity_matrix[i,j]/2.0), str(snp_pi_opportunity_matrix[i,j]/2.0)]
                    record_strs.append( ", ".join(record_str_items) )
        
                    record_str_items = [species_name, sample_i, sample_j, 'all_f2', str(snp_f2_difference_matrix[i,j]/2.0), str(snp_f2_difference_matrix[i,j]/2.0),  str(snp_f2_opportunity_matrix[i,j]/2.0), str(snp_f2_opportunity_matrix[i,j]/2.0)]
                    record_strs.append( ", ".join(record_str_items) )
        
                    record_str_items = [species_name, sample_i, sample_j, 'all_f2variance', str(snp_f2_variance_matrix[i,j]/2.0), str(snp_f2_variance_matrix[i,j]/2.0),  str(snp_f2_opportunity_matrix[i,j]/2.0), str(snp_f2_opportunity_matrix[i,j]/2.0)]
                    record_strs.append( ", ".join(record_str_items) )
