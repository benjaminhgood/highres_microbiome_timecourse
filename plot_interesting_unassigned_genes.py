...
for species_name in ['new_species']:
    ...
    sys.stderr.write("Antibiotic and final: %d\n" % (numpy.logical_and(antibiotic_gene_idxs, final_gene_idxs)).sum())
    sys.stderr.write("Antibiotic and not final: %d\n" % (numpy.logical_and(antibiotic_gene_idxs, numpy.logical_not(final_gene_idxs))).sum())
    
    
    initial_xs = initial_abundances[antibiotic_gene_idxs]*1.0/antibiotic_abundances[antibiotic_gene_idxs]
    
    final_xs = final_abundances[antibiotic_gene_idxs]*1.0/antibiotic_abundances[antibiotic_gene_idxs]
    
    pylab.loglog(initial_xs, final_xs, 'k.', alpha=0.5)
    
pylab.xlabel('Relative initial abundance')
pylab.ylabel('Relative final abundance')
pylab.savefig(parse_midas_data.analysis_directory+'antibiotic_unassigned_genes.png',bbox_inches='tight')