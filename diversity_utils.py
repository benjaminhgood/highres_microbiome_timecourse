...
def calculate_pi_matrix(allele_counts_map, passed_sites_map, allowed_variant_types=set([]), allowed_genes=set([])):
    ...
    pi_matrix = numpy.zeros_like(passed_sites_map[passed_sites_map.keys()[0]]['4D']['sites'])*1.0
    opportunity_matrix = numpy.zeros_like(pi_matrix)
    ...
    for gene_name in allowed_genes:
        ...
        for variant_type in passed_sites_map[gene_name].keys():
            ...
            # site*sample*sample matrix of sites with sufficient coverage in both samples
            passed_depths = (depths>0)[:,:,None]*(depths>0)[:,None,:]
            other_gene_opportunity_matrix = passed_depths.sum(axis=0)
            ...
            gene_opportunity_matrix = numpy.einsum('ij,il',(depths>0)*1.0,(depths>0)*1.0)
    
            #print gene_opportunity_matrix[0,:]
            #print other_gene_opportunity_matrix[0,:]
    
            pi_matrix += gene_pi_matrix
            avg_pi_matrix += gene_avg_pi_matrix
            opportunity_matrix += passed_sites
            
    return pi_matrix, opportunity_matrix

def calculate_f2_matrix(allele_counts_map, passed_sites_map, allowed_variant_types=set([]), allowed_genes=set([])):
    ...
    f2_matrix = numpy.zeros_like(passed_sites_map[passed_sites_map.keys()[0]]['4D']['sites'])*1.0
    opportunity_matrix = numpy.zeros_like(f2_matrix)
    
    f2_variance_matrix = numpy.zeros_like(f2_matrix)
    ...
    for gene_name in allowed_genes:
        ...
        for variant_type in passed_sites_map[gene_name].keys():
            ...
            depths = allele_counts.sum(axis=2)*1.0
            safe_depths = (depths+(depths<0.1))
            safe_depthssquared = (depths*(depths-1)+(depths<1.1))
            
            freqs = allele_counts[:,:,0]*1.0/safe_depths
            freqssquared = (allele_counts[:,:,0]*(allele_counts[:,:,0]-1))*1.0/safe_depthssquared
            
            
            
            # site*sample*sample matrix of sites with sufficient coverage in both samples
            passed_depths = (depths>1.5)[:,:,None]*(depths>1.5)[:,None,:]
            other_gene_opportunity_matrix = passed_depths.sum(axis=0)
 
            I,J = depths.shape
    
            gene_f2_matrix = (passed_depths*(freqssquared[:,:,None]+freqssquared[:,None,:]-2*freqs[:,:,None]*freqs[:,None,:])).sum(axis=0)
    
            gene_f2_variance_matrix = numpy.zeros_like(gene_f2_matrix)
        
            # Term 1 E[A1^2*(A1-1)^2]    
            gene_f2_variance_matrix += ( passed_depths * ( ((depths-2)*(depths-3)/safe_depthssquared*numpy.power(freqs,4))[:,:,None] + (4*(depths-2)/safe_depthssquared*numpy.power(freqs,3))[:,:,None] + (2.0/safe_depthssquared*numpy.power(freqs,2))[:,:,None])).sum(axis=0)
        
            # Term 2 E[A2^2*(A2-1)^2]
            gene_f2_variance_matrix += ( passed_depths * ( ((depths-2)*(depths-3)/safe_depthssquared*numpy.power(freqs,4))[:,None,:] + (4*(depths-2)/safe_depthssquared*numpy.power(freqs,3))[:,None,:] + (2.0/safe_depthssquared*numpy.power(freqs,2))[:,None,:])).sum(axis=0)
        
            # Term 3 E[4*A1^2*A2^2]
            gene_f2_variance_matrix += 4*( passed_depths * ( ((depths-1)/safe_depths*numpy.power(freqs,2)+freqs/safe_depths)[:,:,None]* ((depths-1)/safe_depths*numpy.power(freqs,2)+freqs/safe_depths)[:,None,:])).sum(axis=0)
        
            # Term 4 E[2*A1(A1-1)*A2*(A2-1)]
            gene_f2_variance_matrix += 2*( passed_depths * ( numpy.power(freqs,2)[:,:,None]*numpy.power(freqs,2)[:,None,:] ) ).sum(axis=0)
            
            # Term 5 E[ -4 * A1^2*(A1-1)*A2 ]
            gene_f2_variance_matrix -= 4*( passed_depths * ( ((depths-2)/safe_depths*numpy.power(freqs,3)+2.0/safe_depths*numpy.power(freqs,2))[:,:,None]*freqs[:,None,:] ) ).sum(axis=0)
            
            # Term 6 E[ -4 * A1*A2^2*(A2-1) ] 
            gene_f2_variance_matrix -= 4*( passed_depths * ( ((depths-2)/safe_depths*numpy.power(freqs,3)+2.0/safe_depths*numpy.power(freqs,2))[:,None,:]*freqs[:,:,None] ) ).sum(axis=0)
            
            # Term 7 (subtract mean squared)
            gene_f2_variance_matrix -= (passed_depths*numpy.power((freqs[:,:,None]-freqs[:,None,:]),4)).sum(axis=0)
            
            f2_matrix += gene_f2_matrix
            f2_variance_matrix += gene_f2_variance_matrix
            opportunity_matrix += passed_sites
            
            
    return f2_matrix, f2_variance_matrix, opportunity_matrix
...
# Returns all high coverage samples that are involved in a temporal pair
def calculate_highres_temporal_samples(species_name, min_coverage=config.min_median_coverage):
