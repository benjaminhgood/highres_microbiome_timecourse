...
for species_idx in xrange(0,len(desired_species)):        
    ...
    sys.stderr.write("Loading pre-existing SNVs...\n")
    ...
    if len(alt_matrix)>0:
        ...
        for mutation_idx in xrange(0,len(snp_infos)):
            ...
            #color_condition = color_condition or (masked_freqs[0]>0.5 and ((chromosome,location) not in preexisting_snps))
            
            if masked_freqs[0]>0.5: # and ((chromosome,location) in preexisting_snps):
...
filename = parse_midas_data.analysis_directory+('%s_private_fixation_timecourse.png' % fixation_type)
