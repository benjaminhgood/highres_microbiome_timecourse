...
for species_name in good_species_list:
    ...
    # first see if there are changes between begnning_3 and antibiotic
    ...
    if antibiotic_perr > -0.5 or whole_perr > -0.5:
        if True and (len(antibiotic_changes)>0 or len(whole_changes)>0):
            pass
            #print species_name
        ...
        if species_name.startswith('Alistipes_onderdonkii'):
            pass
            #print "antibiotic 1", antibiotic_1_changes
            #print "antibiotic 2", antibiotic_2_changes
            #print "antibiotic 3", antibiotic_3_changes
            
        
        if len(antibiotic_changes)>0:
        #    print "\t".join([species_name] + [str(change[0]) for change in antibiotic_changes])
            pass
            
        if len(whole_changes)>0:
